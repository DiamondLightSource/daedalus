...
| BEAMLINE_NAME | string | The beamline key string for this configuration entry, it should be a unique identifier for the beamline within Daedalus. The key sting is displayed in the dropdown list of beamlines. |
...
When developing cs-web-lib and Daedalus it is often desirable to run Daedalus using the development version of cs-web-lib. To do this:
...
**To run Daedalus in development mode:**
```
npm run dev
```
...
- Images produced by a manually triggered builds of an untagged commit is not tagged, but will appear as the latest image, until superseded by the next pushed image.


## Demo Synoptic Screens

The B23 synoptic demo requires [techui-builder](https://github.com/DiamondLightSource/techui-builder) to have been run to generated the .bob screen files and jsonMap.json files. These screen files must be deployed onto and served by a webserver. You will need to update the configuration file (`config.json`) ensuring that you create or update the beamline entry for B23 with the correct host and entrypoint parameters.
...
A fileserver can be created using either:
