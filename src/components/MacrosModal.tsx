...
export default function MacrosModal() {
  ...
  function handleOnAddMacroButtonClick() {
    if (nameText !== "" && valueText !== "") {
      dispatch({
        type: ADD_MACROS,
        payload: { name: nameText, value: valueText }
      });
      setValueText("");
      setNameText("");
    }
        ...
        <AccordionDetails>
          <Stack spacing={4}>
            ...
            >
              ...
              <Button variant="contained" onClick={handleOnAddMacroButtonClick}>
                Add Macro
              </Button>
            </Box>
            <TableContainer component={Paper}>
              <Table sx={{ fontSize: "10px" }} aria-label="simple table">
                ...
                <TableBody>
                  {state.nextFile.macros ? (
                    Object.entries(state.nextFile.macros!).map(macro => {
                      return (
                        <TableRow key={macro[0]}>
                          <TableCell align="left">{macro[0]}</TableCell>
                          <TableCell align="left">{macro[1]}</TableCell>
                        </TableRow>
                      );
                    })
                  ) : (
